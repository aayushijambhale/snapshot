  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 pb-32 px-4 sm:px-6">
      ...
      <motion.div
        ...
        whileTap={{ scale: 0.99 }}
        className={`
          relative border-2 border-dashed rounded-[3rem] p-16 text-center transition-all cursor-pointer overflow-hidden
      ...
      >
        ...
        <AnimatePresence>
          {isDragActive && (
            ...
            >
              ...
              >
                <div className="p-8 bg-white dark:bg-neutral-900 rounded-[3rem] shadow-2xl border-2 border-orange-500">
                  <Upload className="w-16 h-16 text-orange-500 animate-bounce" />
                </div>
                <p className="text-3xl font-black text-orange-600 dark:text-orange-400 uppercase tracking-tighter">Drop to Upload</p>
              </motion.div>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="space-y-6 relative z-10">
          <div className="p-6 bg-orange-50 dark:bg-orange-900/20 w-fit mx-auto rounded-[2rem] shadow-lg shadow-orange-100 dark:shadow-none">
            <Upload className={`w-10 h-10 ${isDragActive ? 'text-orange-500' : 'text-orange-500'}`} />
          </div>
          <div className="space-y-2">
            <p className="text-2xl font-black dark:text-white tracking-tight">
              {isDragActive ? 'Drop them now!' : 'Drag & drop photos'}
            </p>
            <p className="text-neutral-500 dark:text-neutral-400 font-medium">or click to browse from your device</p>
          </div>
      ...
      <div className="premium-card p-8 space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-1">
            <h3 className="font-black text-2xl tracking-tight dark:text-white flex items-center gap-3">
              <div className="p-2 bg-orange-500 rounded-xl">
                <Settings className="w-5 h-5 text-white" />
              </div>
              Upload Settings
            </h3>
            <p className="text-sm text-neutral-400 font-medium">Configure how your photos are processed</p>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <button 
              ...
              className="text-xs font-black uppercase tracking-widest text-orange-500 hover:bg-orange-50 dark:hover:bg-orange-900/20 px-4 py-2 rounded-xl transition-all border-2 border-orange-500/20"
            >
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-end">
            <label className="text-[10px] font-black text-neutral-400 uppercase tracking-widest ml-2">Image Quality (Compression)</label>
            <span className="text-xs font-black text-orange-500">{Math.round(compressionSettings.quality * 100)}%</span>
          </div>
          <input 
            type="range" 
            min="0.1" 
            ...
            <AnimatePresence mode="popLayout">
              {photoData.map((photo, index) => (
                <motion.div
                  ...
                  className="premium-card p-6 flex flex-col sm:flex-row gap-8 group relative overflow-hidden"
                >
                  <div 
                    className="relative w-full sm:w-48 aspect-square rounded-[2rem] overflow-hidden flex-shrink-0 shadow-xl cursor-zoom-in"
                  ...
                  >
                    ...
                    {!uploading && (
                      <button
                        ...
                        className="absolute top-3 right-3 p-2 bg-black/50 backdrop-blur-md text-white rounded-xl hover:bg-red-500 transition-all active:scale-90"
                      >
                        <X className="w-4 h-4" />
                      </button>
                    )}
                  ...
                  <div className="flex-1 space-y-6">
                    <div className="flex justify-between items-start">
                      <div className="space-y-1.5 flex-1 mr-4">
                        <label className="text-[10px] font-black text-neutral-400 dark:text-neutral-500 uppercase tracking-widest flex items-center gap-1.5 ml-2">
                        ...
                        <input
                          ...
                          disabled={uploading}
                          className="w-full px-6 py-4 bg-neutral-50 dark:bg-neutral-800 rounded-2xl border-2 border-transparent focus:border-orange-500 focus:bg-white dark:focus:bg-neutral-800 outline-none transition-all text-sm font-bold dark:text-white"
                        />
                      </div>
                      {uploading && (
                        <div className="text-right flex items-center gap-2">
                          {photo.status === 'compressing' && <Loader2 className="w-3 h-3 animate-spin text-orange-500" />}
                          {photo.status === 'uploading' && <RefreshCw className="w-3 h-3 animate-spin text-blue-500" />}
                          {photo.status === 'done' && <CheckCircle2 className="w-3 h-3 text-green-500" />}
                          {photo.status === 'error' && <AlertCircle className="w-3 h-3 text-red-500" />}
            ...
            <div className="max-w-4xl mx-auto">
              {uploading ? (
                <motion.div 
                  initial={{ y: 100 }}
                  animate={{ y: 0 }}
                  className="glass-dark p-8 rounded-[3rem] shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/10 space-y-4"
                >
                ...
                <motion.button
                  ...
                  className="w-full py-6 bg-black dark:bg-white text-white dark:text-black rounded-[2.5rem] font-black text-2xl hover:shadow-2xl transition-all shadow-[0_20px_50px_rgba(0,0,0,0.3)] flex items-center justify-center gap-4 group"
                >
                  <div className="p-2 bg-orange-500 rounded-xl group-hover:rotate-12 transition-transform">
